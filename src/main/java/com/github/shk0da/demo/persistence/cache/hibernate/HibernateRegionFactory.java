...
public class HibernateRegionFactory implements RegionFactory {
    ...
    public void start(SessionFactoryOptions options, Map cfgValues) throws CacheException {
        ...
        String accessType = cfgValues.getOrDefault(
                HibernateAccessStrategyFactory.DFLT_ACCESS_TYPE_PROPERTY, NONSTRICT_READ_WRITE.name()).toString();

        dfltAccessType = AccessType.valueOf(accessType);

        Properties properties = new Properties();
        properties.putAll(cfgValues);
        accessStgyFactory.start(properties);
    }
